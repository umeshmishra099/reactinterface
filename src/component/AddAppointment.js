...
const AddAppointment = ({onSentAppointment, lastId}) => {
    const clearData = {
        ownerName: '',
        petName: '',
        aptNotes: '',
    ...
    function publishDataForm() {
        ...
        onSentAppointment(appointmentInfo);
        setFormData(clearData);
        setToggleAppointment(!toggleAppointment);
    }
    return (<div>
        <button onClick={() => setToggleAppointment(!toggleAppointment)}
                className={`bg-blue-400 text-white px-2 py-3 w-full text-left ${toggleAppointment ? 'rounded-t-md' : 'rounded-md'}`}>
